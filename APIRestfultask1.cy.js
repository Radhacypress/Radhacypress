Cypress.on('uncaught:exception', (err, runnable) =>
   ...
   describe('RestFul Booking Site API Testing', ()=> 
   {
       ...
       .then(function(response){
              ...
              expect(response.body.booking.firstname).equal("Radha")
              expect(response.body.booking.lastname).equal("siva")
              expect(response.body.booking.totalprice).equal(500)
              expect(response.body.booking.depositpaid).equal(true)
              expect(response.body.booking.bookingdates.checkin).equal("2023-10-05")
              expect(response.body.booking.bookingdates.checkout).equal("2023-10-06")
   ...
   .then((response) =>{
    ...
    .then((response)=>{
           expect(response.status).to.eq(200)
           expect(response.body.firstname).equal("Radha")
           expect(response.body.lastname).equal("siva")
   ...
   .then((response) => 
   {
          ...
          expect(response.body.lastname).equal('chennai')
